...
function apply!(o::ADAM, x, Δ)
  ...
  @. mt = β[1] * mt + (1 - β[1]) * Δ
  @. vt = β[2] * vt + (1 - β[2]) * Δ^2
  @. Δ =  mt / (1 - βp[1]) / (√(vt / (1 - βp[2])) + stochϵ) * η
...
function apply!(o::RADAM, x, Δ)
  ...
  if ρ > 4
    ...
    @. Δ =  mt / (1 - βp[1]) / (√(vt / (1 - βp[2])) + stochϵ) * η * r
  else
    @. Δ =  mt / (1 - βp[1]) * η
  end
...
function apply!(o::OADAM, x, Δ)
  ...
  @. vt = β[2] * vt + (1 - β[2]) * Δ^2
  @. Δ = -Δ_
  @. Δ_ = η * mt / (1 - βp[1]) / (√(vt / (1 - βp[2])) + stochϵ)
  @. Δ += 2Δ_
...
function apply!(o::ADAGrad, x, Δ)
  η = o.eta
  acc = get!(o.acc, x, fill!(zero(x), stochϵ))::typeof(x)
  @. acc += Δ^2
  @. Δ *= η / (√acc + stochϵ)
end
...
function apply!(o::AMSGrad, x, Δ)
  η, β = o.eta, o.beta
  mt, vt, v̂t = get!(o.state, x, (fill!(zero(x), stochϵ), fill!(zero(x), stochϵ), fill!(zero(x), stochϵ)))
  @. mt = β[1] * mt + (1 - β[1]) * Δ
  @. vt = β[2] * vt + (1 - β[2]) * Δ ^ 2
  @. v̂t = max(v̂t, vt)
  @. Δ = η * mt / (√v̂t + stochϵ)
...
function apply!(o::NADAM, x, Δ)
  ...
  @. mt = β[1] * mt + (1 - β[1]) * Δ
  @. vt = β[2] * vt + (1 - β[2]) * Δ^2
  @. Δ = (β[1] * mt / (1 - β[1] * β1p) + (1 - β[1]) * Δ / (1 - β1p)) / (√(vt * β[2] / (1 - β2p)) + stochϵ) * η
...
mutable struct ExpDecay
  ...
  step::Int64
  clip::Float64
  current::Base.RefValue{Int}
end

ExpDecay(opt = 0.001, decay = 0.1, decay_step = 1000, clip = 1e-4) = ExpDecay(opt, decay, decay_step, clip, Ref(0))

function apply!(o::ExpDecay, x, Δ)
  η, s, decay = o.eta, o.step, o.decay
  n = o.current[] = o.current[] + 1
  if o.current[] % s == 0
